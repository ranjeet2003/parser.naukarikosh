...
export async function convertAdmitCardHtmlToJson(htmlContent: string): Promise<any> {
  const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });

  const prompt = `Convert the following HTML content from an admit card or exam update page into a structured JSON object.
